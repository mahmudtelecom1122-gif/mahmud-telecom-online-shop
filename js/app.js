...
db=normaliseState(db);
function save(){
  try{db=normaliseState(db);localStorage.setItem(KEY,JSON.stringify(db));localStorage.setItem(KEY+'_updated',String(Date.now()));}catch(e){console.error('Local save failed',e);toast('লোকাল হিসাব সংরক্ষণ করা যায়নি');return false;}
...
// Dedicated Mobile Banking safety handlers
document.addEventListener('click',e=>{
  const del=e.target.closest('[data-delete-bank]');
  if(del && del.dataset.handledBankDelete!=='1'){
    del.dataset.handledBankDelete='1';
    if(confirm('এই লেনদেনটি মুছে ফেলবেন?')){db.banking=db.banking.filter(x=>x.id!==del.dataset.deleteBank);save();toast('মোবাইল ব্যাংকিং হিসাব মুছে ফেলা হয়েছে');}
  }
});
